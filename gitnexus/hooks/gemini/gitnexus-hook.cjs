#!/usr/bin/env node
/**
 * GitNexus Gemini CLI Hook
 *
 * BeforeTool  — intercepts search/shell tools and augments with graph context
 *               from the GitNexus index.
 * AfterTool   — detects stale index after git mutations and notifies the agent
 *               to reindex.
 *
 * Gemini CLI tool names (snake_case):
 *   run_shell_command    → shell execution (Bash equivalent)
 *   search_file_content  → regex file search (Grep equivalent)
 *   list_directory       → directory listing (Glob equivalent)
 */
...
/**
 * Extract search pattern from tool input (Gemini tool names).
 */
function extractPattern(toolName, toolInput) {
  if (toolName === 'search_file_content') {
    return toolInput.pattern || toolInput.query || null;
  }

  if (toolName === 'list_directory') {
    // Extract a meaningful name fragment from the path
 ...
 * Gemini CLI: stdout must be valid JSON only.
 */
function sendHookResponse(message) {
  console.log(JSON.stringify({ output: message }));
}

/**
 * BeforeTool handler — augment searches with graph context.
 */
function handleBeforeTool(input) {
  ...
  if (result && result.trim()) {
    sendHookResponse(result.trim());
  }
}

/**
 * AfterTool handler — detect index staleness after git mutations.
 *
 * Does a lightweight staleness check: compare `git rev-parse HEAD` against the
 * lastCommit stored in `.gitnexus/meta.json`. If they differ, notify the agent.
 */
